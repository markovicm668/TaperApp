...
function Card({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      data-slot="card"
      className={cn(
        "bg-card text-card-foreground flex flex-col gap-5 rounded-2xl border border-border/85 py-5 shadow-[0_1px_1px_rgba(15,23,42,0.06),0_6px_18px_rgba(15,23,42,0.035)]",
        className
      )}
      {...props}
    />
...
function CardContent({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn("px-6 pb-1", className)}
      {...props}
    />
  )
}
